...
class CubicDetectorDriver extends Driver {
  ...
  async onInit(): Promise<void> {
    this.log("CubicDetector driver has been initialized");
  }
  ...
  private async onListDevices(): Promise<RealEstateSelection[] | DeviceItem[]> {
    ...
    this.listView = 1;
    return await this.onPairListCubicDetectorDevices();
  }
  ...
  private async onPairListCubicDetectorDevices(): Promise<DeviceItem[]> {
    this.log("onPairListCubicDetectorDevices()");
    ...
    try {
      ...
      const cubicDetectorDevices = devices.filter(
        (device) =>
          device.deviceType?.toLowerCase() === "cubicdetector" &&
...
module.exports = CubicDetectorDriver;
