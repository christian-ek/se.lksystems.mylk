...
class LkDevice extends Homey.Device {
  ...
  async getDeviceData() {
    ...
    const urlBase = `http://${Homey.ManagerSettings.get('ip')}/thermostat.json?tid=`;
  ...
  async updateCapabilityValues(capability) {
    const { device } = this;
    try {
      const urlBase = `http://${Homey.ManagerSettings.get('ip')}`;
      const password = Homey.ManagerSettings.get('password');
      const auth = `Basic ${Buffer.from(`lk:${password}`).toString('base64')}`;
      let url;
  ...
  async onSettings(oldSettings, newSettings) {
    ...
    if (oldSettings.interval !== newSettings.interval) {
      this.log(`Delete old interval with ${oldSettings.interval} min and creating new ${newSettings.interval} min`);
